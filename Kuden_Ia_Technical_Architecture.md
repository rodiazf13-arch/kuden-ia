...
*   `llmService.js`: Encargado de hablar con las APIs de LLM (OpenAI, Anthropic, Gemini, Groq, OpenRouter). Antes de realizar cualquier llamada, consulta `llmBillingService.js` para aplicar la **Regla de Oro de Seguridad (Binding Constraint)**: si el proveedor no está activo en `provider_keys` o el modelo está inactivo/ausente en `llm_models_pricing`, arroja un error 403 impidiendo la petición y evitando fugas de costos o modelos no autorizados. Al responder, calcula e inserta en `usage_logs` los tokens entrantes/salientes y costos asociados.
*   `llmBillingService.js`: Servicio desacoplado para gestión de tarificación y consumo granular. Encapsula la lógica de validación previa (`validateProviderAndModel`), el cálculo transaccional del costo en USD (`calculateCost`) y el registro en la tabla transaccional `usage_logs`.
