...
*   `server.js`: El monolito. Inicializa Express, carga CORS, expone todas las rutas (`/api/crm`, `/api/chat`, `/api/auth`, `/api/webhook`, y `/api/master`) e interactúa con el SDK de Supabase. Implementa el **Gatekeeper Transaccional** para validar action locks antes de invocar herramientas en n8n. Contiene hooks asíncronos como `generateExecutiveSummary` (que actúa como la auditoría de calidad **"Shadow Kimi"**) y `generateRAGSuggestion` para operaciones post-cierre y aprendizaje auto-didacta. Adicionalmente, auto-asigna conversaciones entrantes sin dueño al primer ejecutivo humano que responda e intercepta errores de seguridad `BINDING_CONSTRAINT_VIOLATION` retornando HTTP `403 Forbidden` controlado con JSON explícito para el frontend y widget.
...
*   `llmService.js`: Encargado de hablar con las APIs de LLM (OpenAI, Anthropic, Gemini, Groq, OpenRouter). Antes de realizar cualquier llamada, consulta `llmBillingService.js` para aplicar la **Regla de Oro de Seguridad (Binding Constraint)**: si el proveedor no está activo en `provider_keys` o el modelo está inactivo/ausente en `llm_models_pricing`, arroja un error 403 impidiendo la petición. Además, envuelve todas las llamadas LLM en bloques `try/catch` auditados que insertan de inmediato cualquier fallo o intercepción en la bitácora `audit_logs` del **Health Monitor**.
*   `llmBillingService.js`: Servicio desacoplado para gestión de tarificación y consumo granular. Encapsula la lógica de validación previa (`validateAndGetModelPricing`), el cálculo transaccional del costo en USD (`calculateAndLogLlmCost`) y el registro en `usage_logs`. Expone `logSystemHealth` como puente oficial transaccional hacia `audit_logs` para reportar anomalías matemáticas, caídas o violaciones del Gatekeeper al **Health Monitor**.
*   `routes/llmPricing.js`: Capa REST para la administración y consulta del Catálogo & Tarifas (`llm_models_pricing`). Expone `/api/master/provider-models` y `/api/master/llm-models`. Incorpora un mecanismo de **Auto-Población Inteligente (Auto-Seed 2026)**: al ser consultado, si el proveedor solicitado carece de modelos en la base de datos o la tabla se encuentra vacía, inserta automáticamente (`upsert`) el catálogo oficial 2026 de Kuden QA (`gpt-5.5`, `claude-sonnet-4-6`, `gemini-3.1-pro-preview`, etc.), garantizando selectores sincronizados e impidiendo la degradación a modelos obsoletos.
...
*   `/api/master/*`: Router dedicado (`routes/llmPricing.js`) montado en `/api/master` para la gestión administrativa centralizada de proveedores y modelos LLM. Incluye `GET/PUT /provider-keys` (gestión de llaves e interrupción por `is_active`), `GET/POST/PUT /pricing` y `/models` (CRUD de tarifas y control de activación por modelo) y `GET /provider-models` (catálogo filtrado para selectores dinámicos en frontend).
...
*   `provider_keys`: Bóveda centralizada de credenciales LLM. Almacena `provider_name` (`anthropic`, `openai`, `gemini`, `groq`, `openrouter`), `api_key` (encriptada lógicamente en bóveda maestra), `base_url`, e `is_active` para la interrupción global inmediata de un proveedor.
*   `llm_models_pricing`: Mantenedor corporativo de modelos y tarifas USD. Almacena `provider`, `model_name`, `friendly_name`, `prompt_rate` y `completion_rate` (costo por 1M de tokens), `is_active` (control de acceso del Binding Constraint) e `is_router_capable`.
*   `usage_logs`: Bitácora transaccional granular del consumo de IA. Almacena `id`, `tenant_id`, `campaign_id`, `ai_profile_id`, `provider`, `model`, `prompt_tokens`, `completion_tokens`, `cost_usd` (costo neto calculado con `llm_models_pricing`), `billed_usd` (costo facturado al cliente usando el margen o markup), `source` (`chat`, `widget`, `rag`), y `created_at`. Incluye la vista retrocompatible `llm_usage_logs` que expone `api_cost_usd` mapeado a `cost_usd`.
