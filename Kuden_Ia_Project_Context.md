...
*   ✅ **Gestión APIs y LLM (Bóveda Central & Tarificador Corporativo):** Evolución estructural de grado corporativo de la gestión de APIs e IA (Kuden QA Standard). Se desacopló por completo el hardcoding de modelos y precios implementando dos tablas maestras en Supabase (`provider_keys` con encriptación lógica en bóveda central y `llm_models_pricing` como mantenedor maestro con tarifas oficiales USD por 1M de tokens input/output y control granular `is_active`). Se incorporó la **Regla de Oro de Seguridad (Binding Constraint)** interceptando desde el servidor (`llmService.js`) cualquier llamada a la IA si el proveedor o el modelo específico se encuentra desactivado o no existe en el catálogo (`[403 Forbidden - BINDING_CONSTRAINT_VIOLATION]`), protegiendo de consumos indebidos y alertando de forma limpia en el Agente Web y el CRM. Además, el consumo granular se registra en la tabla transaccional `usage_logs` calculando costos en tiempo real y exponiéndose en un panel `BillingDashboard.jsx` rediseñado, que incluye soporte retrocompatible mediante la vista `llm_usage_logs` y exportación integral para facturación por empresa.
...
2.  ✅ **Gestión APIs y LLM (Bóveda Central & Tarificador Corporativo & Health Monitor):** Conexión multi-proveedor (OpenAI, Gemini, Claude, Llama, OpenRouter) con desacoplamiento total de tarifas vía tablas maestras (`provider_keys`, `llm_models_pricing`, `usage_logs`). Incluye intercepción por seguridad **Regla Binding Constraint (403 Forbidden)** para evitar consumos de modelos inactivos y panel de margen comercial por empresa (`BillingDashboard.jsx`). **COMPLETADO.**
    *   **Resiliencia Total y Normalización en Tarificador (PostgREST Error 400):** Se eliminó el join embebido `tenants ( name )` y se migró la consulta de proyección a **`.select('*')`** tanto para la visualización en vivo como en la exportación CSV en `BillingDashboard.jsx`. Un middleware interno en JS (`normalizeLogItem`) homogeneiza de forma transparente diferencias en columnas de tablas/vistas (`model_name` vs `model`, `feature_type` vs `source`, `api_cost_usd` vs `cost_usd`, etc.), eliminando permanentemente errores relacionales o 400 en navegadores y caché de Service Worker (`sw.js`).
    *   **Monitoreo Integral LLM (`audit_logs` / Health Monitor):** Se integró una capa de observabilidad transaccional en `llmService.js` y `llmBillingService.js`. Todo intento bloqueado por la Regla de Oro (Gatekeeper 403), caída de proveedor API o error matemático de tarificación queda registrado automáticamente en `audit_logs` con severidad `error`/`warning` y trazabilidad completa para inspección desde el panel de **Health Monitor**.
    *   **Auto-Población Inteligente y Catálogo 2026 Oficial:** Se actualizó en `llmPricing.js` el catálogo oficial con las tarifas y modelos 2026 (`gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.4-nano`, `claude-opus-4-8`, `claude-sonnet-4-6`, `gemini-3.1-pro-preview`, `gemini-2.5-pro`, `gemini-3.5-flash`, `llama-4-70b-8192`, `llama-4-8b-8192`). Si la tabla `llm_models_pricing` no contiene aún los modelos modernos para un proveedor al entrar a **Catálogo & Tarifas**, **Identidad Maestra** (`AIConfigManager.jsx`) o **Perfiles IA** (`ProfilesManager.jsx`), el backend realiza de forma automática un **`upsert` silencioso en PostgreSQL/Supabase**, sincronizando al instante la base de datos y garantizando que los selectores de los bots muestren siempre las versiones más recientes.
