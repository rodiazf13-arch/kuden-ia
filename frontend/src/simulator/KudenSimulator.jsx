        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {CHANNELS.map(ch => (
            <button key={ch.id} onClick={() => setCanal(ch.id)}
              style={{
                display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 20, cursor: "pointer", fontSize: 11,
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 10, borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{
              padding: "6px 12px", fontSize: 12, fontWeight: tab === t.id ? 500 : 400, cursor: "pointer", background: "none", border: "none",
                ...
                <div style={{ display: "flex", borderBottom: "0.5px solid var(--color-border-tertiary)", background: "var(--color-background-secondary)", padding: "0 12px" }}>
                  {CHANNELS.map(ch => (
                    <button key={ch.id} onClick={() => { if (!closedCase) { setCanal(ch.id); canalRef.current = ch.id; } }}
                      style={{
                        display: "flex", alignItems: "center", gap: 4, padding: "7px 10px", fontSize: 11, cursor: closedCase ? "not-allowed" : "pointer", background: "none", border: "none",
                      ...
                      <div style={{ display: "flex", justifyContent: m.role === "assistant" ? "flex-start" : "flex-end" }}>
                        {m.role === "assistant" && (
                          <div style={{ width: 22, height: 22, borderRadius: "50%", background: "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginRight: 5, alignSelf: "flex-end" }}>
                            <KimiMascot size={42} state={loading ? "thinking" : "idle"} />
                          </div>
                        )}
                        <div style={{
                          ...
                          color: m.role === "assistant" ? "var(--color-text-primary)" : "var(--color-text-primary)"
                        }}>{m.text}</div>
                      </div>
                  ...
                  {loading && (
                    <div style={{ display: "flex", alignItems: "flex-end", gap: 5 }}>
                      <div style={{ width: 22, height: 22, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <KimiMascot size={42} state="thinking" />
                      </div>
