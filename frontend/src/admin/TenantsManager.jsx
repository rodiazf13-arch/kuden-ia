...
export default function TenantsManager({ isDark = true }) {
  ...
  const [n8nEmailWebhook, setN8nEmailWebhook] = useState('');
    ...
    try {
      ...
      if (editingTenantId) {
        ...
        const { error: updErr } = await supabase.from('tenants')
          .update({ name, industry_type: industry, website, address, is_active: isActive, llm_markup_multiplier: parseFloat(llmMarkup) || 1.20, logo_url: logoUrl, primary_color: primaryColor, n8n_outbound_email_webhook: n8nEmailWebhook || null })
        ...
        const { data, error } = await supabase.from('tenants')
          .insert([{ name, industry_type: industry, website, address, is_active: isActive, llm_markup_multiplier: parseFloat(llmMarkup) || 1.20, logo_url: logoUrl, primary_color: primaryColor, n8n_outbound_email_webhook: n8nEmailWebhook || null }])
          .select();
        if (error) throw error;
      ...
      fetchTenants();
      setName(''); setIndustry('general'); setWebsite(''); setAddress(''); setIsActive(true); setLlmMarkup('1.20'); setLogoUrl(''); setPrimaryColor('#2563eb'); setSelectedFields([]); setCustomFields([]); setN8nEmailWebhook('');
  ...
  const openEdit = async (t) => {
    ...
    setN8nEmailWebhook(t.n8n_outbound_email_webhook || '');
  ...
  const startCreate = () => {
    setEditingTenantId(null);
    setName(''); setIndustry('general'); setWebsite(''); setAddress(''); setIsActive(true); setLlmMarkup('1.20'); setLogoUrl(''); setPrimaryColor('#2563eb'); setN8nEmailWebhook('');
      ...
      <form onSubmit={handleSave}>
        ...
        <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontSize: '12px', color: c.label }}>Webhook Outbound n8n (Email)</label>
              <input type="url" value={n8nEmailWebhook} onChange={e => setN8nEmailWebhook(e.target.value)} placeholder="Ej: https://n8n.kuden.cl/webhook/..." style={inputStyle} />
            </div>
